...
/// arguments, hands the work to the core, and returns a [`PyDriverFuture`]
...
impl PySession {
    ...
    fn use_keyspace(
        ...
        PyDriverFuture::spawn_on_tokio(py, self.core.clone().use_keyspace(keyspace, case_sensitive))
    }
    ...
    fn execute(
        ...
        PyDriverFuture::spawn_on_tokio(py, async move { request.await.map(RequestResult::from) })
    }
    ...
    fn prepare(
        ...
        PyDriverFuture::spawn_on_tokio(py, self.core.clone().prepare(statement))
    }
    ...
    fn batch(
        ...
        PyDriverFuture::spawn_on_tokio(py, async move {
            core.batch(batch, factory).await.map(RequestResult::from)
        })
    }

    fn await_schema_agreement(&self, py: Python<'_>) -> PyResult<Py<PyDriverFuture>> {
        PyDriverFuture::spawn_on_tokio(py, self.core.clone().await_schema_agreement())
    }

    fn check_schema_agreement(&self, py: Python<'_>) -> PyResult<Py<PyDriverFuture>> {
        PyDriverFuture::spawn_on_tokio(py, self.core.clone().check_schema_agreement())
    }
