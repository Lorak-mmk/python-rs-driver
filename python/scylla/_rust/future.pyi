...
class DriverFuture(Generic[_T]):
    ...
    When callbacks are registered via :meth:`on_result` or :meth:`on_error`,
    the future becomes **eager**: it is spawned on a background thread and
    driven to completion without requiring ``await``. Callbacks are invoked
